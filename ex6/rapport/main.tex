...
\section{Differential equation and variational form}
  ...
  \subsection{Special case: cylinder} %TODO : Faut changer le titre, il pue.
    ...
    Note that a factor $r$ appears inside the integral, coming from the change of variable into cylindrical coordinates.
...
\section{Finite elements}
  ...
  \subsection{Computation of the polarization charge density $\rho_{pol}$} \label{sec:rhopol}
    ...
    The polarization charges are the charges on the dielectrics that can not move, which forms a multitude of electrical dipoles.
    The density of polarization charges can be found using equations \eqref{eq:def-polarization} and \eqref{eq:def-rhopol}. \cite{feynman:dielectrics}
  ...
  The simplest way to obtain this area is to compute the area of the triangle surrounding $r=b$ in figure \ref{fig:rhopol}.
  ...
  It shows that the value of the polarization charge converges to the first order.
  ...
  \section{Supplement} %TODO : Le titre de la section pue.
    \subsection{Combination of the trapezoidal rule and the middle point rule} \label{sec:trapezoidal-middle-rule}
      ...
      To achieve this, the trapezoidal rule was used up until now.
      ...
      Then, the trapezoidal-middle point rule \cite{cours} is given by equation \eqref{eq:int-trapeze-milieu}, where $p\in[0,1]$ is a parameter.
