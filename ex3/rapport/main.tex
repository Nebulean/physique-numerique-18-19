...
\end{equation}
Since we want the movement to be similar to that of the pendulum with $\theta \approx 0$, we set $\theta_2$ one such small angle and equalize the accelerations:
\begin{align}
...
\centering
	...
	\caption{Stabilization of angle $\theta$ around $\pi$ with damping coefficient $\kappa = 0.1$}
	\end{subfigure}
...
\begin{figure}[h]
	...
	\end{subfigure}
	\caption{Poincaré sections showing differents types of strange attractors, with different situations.} %TODO : Il est pas ouf celui-là, je vais peut-être le changer.
