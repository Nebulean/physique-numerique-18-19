...
This experiment will focus on the study of a simple pendulum in a box oscillating with frequency $\Omega$ and amplitude $d$.
...
\subsection{Solutions for small movements around equilibrium}\label{sec:sol-analytique}
...
\end{equation}
with $A$ the amplitude and $\phi$ the phase.
...
Figure \ref{fig:a-conv} show the study of the error on the final angle with respect to the time step.
...
\begin{figure}[h!]
	...
	\caption{Verification of the mechanical energy theorem by drawing the difference of the derivative of the mechanical energy and the power of non-conservative forces over time}
	\label{fig:c-thm}
\end{figure}

As expected the difference $\frac{d(E_m)}{dt}-P_{nc}$ is almost zero everywhere. The maximum error of order $10^{-3}$ is caused by the imperfect numerical derivation of $E_m$ at times when it varies quickly.
...
Figure \ref{fig:c-emax} shows that the system has very little response to the excitation for $\Omega<9.749$. However the maximum of the response in energy then increases exponentially until it peaks around $\Omega= 9.74985$, an excitation frequency lower than the natural frequency of the system, $\omega_0$. The maximum of the energy then decreases linearly until no response is observable from $\Omega=9.96$ onwards.
...
Figure \ref{fig:d-thm} is the verification of the mechanical energy theorem, where the derivative of $E_{m}$ has been computed numerically.
...
It can be seen that the maximum reached by the mechanical energy increases exponentially, until a point where it just decreases linearly.
...
\centering
	...
	\begin{subfigure}[t]{0.45\textwidth}
		...
		\caption{Evolution of the mechanical energy $E_{m}$ for 5 different $\Omega$}
...
%TODO : Il faut comparer les deux cas.
...
\subsubsection{Stabilization around $\theta=\pi$}

In this section we will try to find $\Omega$ and $d$ such that $\theta$ stabilizes around $\pi$. We start with the movement equation for small movements with $d=0$ and $k=0$ (eq.\eqref{eq:equa-diff-no-box}) and write the equivalent for small movements around $\pi$ with $d \neq 0$, using the small angle $\phi := \theta_1-\pi$:
\begin{equation}
\ddot{\theta_1}=\ddot{\phi} = \frac{g}{L}\sin\phi - \frac{\Omega^2}{L}d\sin(\Omega t) \sin\phi
\end{equation} since $\sin(\phi+\pi) = -\sin(\phi)$.
Then by using the approximation $\sin\phi \approx \phi$ for small angles:
\begin{equation}
\ddot{\phi} = \frac{g}{L}\phi -\frac{\Omega^2}{L}d\sin(\Omega t)\phi
\end{equation}
Since we want the movement to be similar to that of the pendulum with $\theta \approx 0$, we set $\theta_2$ one such small angle and equalize the accelerations using eq.\eqref{eq:equa-diff-no-box}:
\begin{align}
\ddot{\theta_2} \approx \ddot{\phi}\\
-\frac{g}{L} \theta_2 \approx \frac{g}{L}\phi -\frac{\Omega^2}{L}d\sin(\Omega t)\phi
\end{align}
And we choose $\theta_2=\phi$ since they are both small angles. Thus we get:
\begin{equation}
\Omega^2 d \sin(\Omega t) \approx 2g
\end{equation}

As an approximation we then choose $\Omega=\sqrt{2g}/d$ and vary $d$ to empirically find a system with $\theta_0$ close to $\pi$ which oscillates around $\pi$. For $\theta_0=pi-0.01 ~\si{\radian}$, $\dot{\theta}_0=0~\si{\radian \per \s}$, $t_{end}=20s$ and $d=0.035m$, we get figure \ref{fig:g-stab}.

\begin{figure}[h]
\centering
	\begin{subfigure}[t]{0.49\textwidth}
	\includegraphics[width=\textwidth]{graphs/g_stab.eps}
	\caption{Oscillation of angle $\theta$ around $\pi$}
	\end{subfigure}
	\begin{subfigure}[t]{0.49\textwidth}
	\includegraphics[width=\textwidth]{graphs/g_stabk.eps}
	\caption{Stabilization of angle $\theta$ around $\pi$ with damping coefficient $\kappa = 0.1 ~ \si{\kg \per \square \s}$}
	\end{subfigure}
	\caption{Stabilization of angle $\theta$ around $\pi$}
	\label{fig:g-stab}
\end{figure}

However it should be noted that the stabilization worked not only with these precise conditions but also with $0.03m\leq d \leq 0.04m$, or with $\theta_0$ further away from $\pi$, or even with $\Omega=\sqrt{\pi g}/d$.
...
In the case of $\theta$, these low frequencies have a bigger amplitude than the peaks, whereas $\dot{\theta}$, these low frequencies are much smaller.
Some frequencies are clearly described, but some other are muddled with others, which is what was expected for chaotic situations.
...
On figure \ref{fig:g-funfunfun-eye}, points are attracted by the eye-shaped form.
On figure \ref{fig:g-funfunfun-rainbow}, points are attracted by the curve.
By changing $\Omega$, the attractor just moves, and points follow.
On figure \ref{fig:g-funfunfun-sun}, points are attracted by the sun-shaped form.
All of these are strange attractors of chaotic situations.
...
\begin{figure}[h!]
	...
	~\newline
	\begin{subfigure}[t]{0.43\textwidth}
		\includegraphics[width=\textwidth]{graphs/g_funfunfun_rainbow.eps}
		\caption{Seven simulations were done to compare the changes of the strange attractor with respect to $\Omega$, where $\Omega\in\left[\num{14.36}, \num{16.44}\right]$ and each color is a simulation in this range. Conditions: $\kappa=0.0001$, $d=0.001$, $\theta_0=\pi-\num{1d-8}$ and $\dot{\theta}_0=10$.}
		\label{fig:g-funfunfun-rainbow}
	\end{subfigure}
	~
	\begin{subfigure}[t]{0.43\textwidth}
		\includegraphics[width=\textwidth]{graphs/g_funfunfun_sun.png}
		\caption{Conditions: $\kappa=0.001$, $d=0.01$, $\theta_0=0$, $\dot{\theta}_0=\num{1d-2}$ and $\Omega=0.8\omega_0$.}
		\label{fig:g-funfunfun-sun}
	\end{subfigure}
	\caption{Poincaré sections showing differents types of strange attractors, with different situations. $t_{end}$ is computed by $\num{30000}\times\frac{2\pi}{\Omega}$} %TODO : Il est pas ouf celui-là, je vais peut-être le changer.
	\label{fig:g-funfunfun}
\end{figure}
% \begin{figure} %TODO : On a pas le mettre, ça n'a rien à faire dans ce rapport.
% 	\centering
% 	\includegraphics[width=0.6\textwidth]{graphs/g_funfunfun_rainbow.eps}
% 	\caption{\text{The rainbow}: }
% \end{figure}



\begin{thebibliography}{99}
	\bibitem{ans:moment-inertie} Ansermet, J-P. (2015) \textit{Traité de Physique: Mécanique}. Lausanne, Vaud: Presses Polytechniques et Universitaires Romandes.
