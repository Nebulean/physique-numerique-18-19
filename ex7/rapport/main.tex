...
\section{Implementation}
  ...
  \subsection{Energy of the wave}
    ...
    The result for the \textbf{fixed} right border condition is represented on figure \ref{fig:ex1fix}. In this case the wave reflected off both sides and superposed with the wave coming from the left, such that the maximum energy of the resulting wave kept growing slowly. When reflecting on the right side, the wave turned ``upside-down'', such that it had a phase of $\phi=\pi$ compared to the original wave. The sharp corner visible on the shape of the wave at $t_{fin} = 14.01$ s (Fig. \ref{fig:ex1fixf}), around $x=4$ m is the front of the wave, which keeps propagating as the waves coming from both directions are not in phase, and thus don't interfere completely constructively or destructively. This is because the frequency of excitation $\omega=5$ rad/s is not an eigenfrequency of the system. This is also why the maximum amplitude of the wave does not grow very much after the first round trip is completed, as seen on figure \ref{fig:ex1fixxt}.
    ...
    \section{Tsunami}
      ...
      The simulation will consider a single sine wave of initial amplitude \SI{1}{\meter} coming from the left border, which has a period of \SI{15}{\min} and exits on the right border.\\
      ...
      \subsubsection{Propagation speed}
        ...
        \begin{enumerate}
          ...
          This will give the location of the maximum of the first wave on the discretized system.
          \item Now, the problem is that the found maximum is only that of the the discretized system, and not the real maximum.
          The real maximum is not numerically findable, thus it needs to be approximated through an interpolation.
          ...
          To do so, consider the last three points of the domain.
          Find the best line that fits these points, and consider its slope.
          When the slope is null, no movement is detected.
          When it is negative, a wave is starting its exit.
          When it is positive, a wave is more that halfway out of the domain.
          Thus, the measurements on the first wave need to be stopped when the slope becomes positive.
        ...
        This process leads to figure \ref{fig:tsunami_speed}, which gives a comparison between the theoretical propagation speed of a wave, and the three equations considered.

        \begin{figure}[h]
          \centering
          \includegraphics[width=\textwidth]{graphs/tsunami_speed.eps}
          \caption{Comparison between the theoretical value of the propagation speed, and the three equations for $t_\text{fin} = \SI{10000}{\s}$ and $N_\text{points} = \num{1000}$.}
          \label{fig:tsunami_speed}
        \end{figure}

        All equations tends to follow the theoretical propagation speed, but they are different from each other.
        Indeed, equation C seems to be the least well-suited of them.
        It follows the global trend, but when the depth of water changes, the propagation speed moves further apart from the theoretical value, before slowly stabilizing around it.
        Regarding equations A and B, the results are similar, as both closely follow the theoretical propagation speed.
        It can be also observed that, before reaching the reef, all equations give an oscillation solution.
        When leaving the reef, all equations give a smoother result than before. %TODO : Ca vient d'où ça ? Une idée ? Faut dire un truc sinon ça sert à rien. -> ASSISTANT
      ...
      While on the reef, the amplitude of the wave slowly decreases.
      This decrease leads to a gap between the numerical solution and the WKB solution when the wave leaves the reef.\\

      The problem may come from the WKB approximation, which makes multiple hypotheses that may not be entirely verified in the case of the tsunami.
      This method considers a general solution of the problem given by $f(x,t)=e^{-i\omega t}\hat{f}(x)$ with $\hat{f}(x) = A(x)e^{iS(x)}$.
      Two main hypotheses are therefore considered: $A(x)$ (amplitude) varies slowly, and $S(x)$ (phase) varies quickly.
      ...
      Equation A (Fig. \ref{fig:tsunami-amp-A}) gives a reduction of the amplitude of a wave when approaching the reefs, which is not what happens for real tsunamis.
      ...
      Thus, either our wave was created by a much more energetic event, or equation C is unsuitable for this problem.\\

      To summarize, the verifications of all three equations by the amplitudes and the propagation speed lead to the following conclusion: equation B is the only equation that gives realistic results.
      ...
      \subsubsection{Propagation speed}
        ...
        Approaching $x_a$ to $x_b$ increase the peak of speed around $x_a$.
        ...
        This imprecision can be reduced by increasing the number of points considered, which will reduce the distance and time step, but also drastically increase the computation time of each simulation.
      ...
      \subsubsection{Amplitude}
        ...
        Figure \ref{fig:xa-WKB-250km} shows that the numerical and the WKB solutions are not so far from each other, but figure \ref{fig:xa-WKB-350km} shows that the WKB solution is not a good approximation anymore, as its maximum is almost twice as big as the numerical solution.
        Figure \ref{fig:xa-WKB-100m} shows that, by moving $x_a$ away from $x_b$, which reduces the variation of the amplitude, the WKB solution gets really close to the numerical solution.
...
\section{Optional}
  \subsection{WKB analysis}\label{sec:WKB}
    This section give the WKB analysis for each equation.
    Let $f(x,t)$ be defined by $f(x,t) = \hat{f}(x)e^{-i\omega t}$, with $\hat{f}(x) = A(x)e^{iS(x)}$, where $A(x)$ varies slowly, and $S(x)$ varies quickly.
    Note that the development for equation B (Sec. \ref{sec:WKB-B}) is more detailed, as it is the most important equation for the report.
    ...
    \subsubsection{Equation B}\label{sec:WKB-B}
      \begin{align*}
        ...
        \intertext{Now, the elements need to be grouped by "variation order". This variation order $n$ is defined by $\epsilon^n$, and $n$ increases by $1$ each time a derivation occurs. Also, note that $\epsilon$ works the same as any real number, which means $\epsilon^n\cdot\epsilon^m = \epsilon^{n+m}$. Thus, by hypothesis, let $A(x)\sim\epsilon^0$ and $S'(x)\sim\epsilon^0$. This leads to:}
    ...
    \begin{figure}[h]
     ...
     \caption{Numerical error for $f(5\text{m},1.5\text{s})$ with respect to $\beta_{CFL}$ for values inside and outside of the limits of stability}
     \label{fig:unstable}
    \end{figure}
