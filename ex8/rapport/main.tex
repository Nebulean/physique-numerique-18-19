...
\section{Implementation}\label{sec:impl}
  ...
  \subsection{Semi-implicit Crank-Nicholson's numerical method}
    ...
    By multiplying both sides by $\exp\bracket{\frac{i}{\hbar}\frac{\Delta t}{2}H}$, and by using a limited development, equation \eqref{eq:crank-nicholson} follows, which is Crank-Nicholson's semi-implicite numerical method.
  ...
  \subsection{Construction of matrices H, A and B and implementation of the border conditions}
    \subsubsection{Construction of H}\label{sec:constr_H}
      ...
      By discretizing the space using a finite difference method (Eq. \eqref{eq:disc_space}), the hamiltonian can be rewritten as a tridiagonal matrix given by \eqref{eq:hamiltonian_matrix}, where $d_i$ is the diagonal, $a_i$ is the subdiagonal and $c_i$ is the overdiagonal.
    ...
    \subsubsection{Construction of A and B}
      In Crank-Nicholson's semi-implicite method (Eq. \eqref{eq:crank_nicholson}), and by using the matrix H, matrices A and B can be identified as the left hand side respectively the right hand side.
  ...
  \subsection{Energy}
    ...
    To compute this energy, the trapezoidal rule will be used.\\
    ...
    with $h = \Delta x$.
    Thus, by using the results from equation \eqref{eq:hamiltonian_matrix}, the vector definition of $\psi$ and the trapezoidal rule, the energy can be computed.
